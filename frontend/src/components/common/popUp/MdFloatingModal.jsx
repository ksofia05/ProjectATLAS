...
const MdFloatingModal = ({ children, onClose, showClose = true, open = true, title }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 pointer-events-none">
      ...
      >
        <div
          className="border border-gray-700 relative bg-gradient-to-tr from-[#17161d] via-[#1f1e29] to-[#323342] rounded-2xl shadow-2xl px-10 py-8 max-w-xl w-full scrollbar-none"
          style={{
            minHeight: "220px",
            maxHeight: "600px",
            overflowY: "auto",
            scrollbarWidth: "none",
            // Responsivo para pc más pequeñas
            ...(window.innerWidth < 640 ? { maxHeight: "60vh" } : {}),
          }}
        >
          {showClose && onClose && (
            <button
              onClick={handleClose}
              className="absolute top-3 right-3 text-purple-400 hover:text-purple-600 text-2xl font-bold"
              aria-label="Cerrar"
            >
              ✕
            </button>
          ...
          {title && (
            <>
              <h1 className="text-2xl font-bold text-white mt-2 mb-2 text-center">{title}</h1>
